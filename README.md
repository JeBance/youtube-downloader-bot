...
Бот умеет искать видео по ключевым словам! Просто отправьте текстовый запрос, например:
...
**Выбор языка:**
Используйте команду `/lang` для выбора предпочтительного языка поиска:
...
| `/lang` | **Выбор языка поиска** (русский/English) |
...
├── database.py             # Кэш SQLite + настройки пользователей
├── queue_manager.py        # Менеджер очереди загрузок (Fair Queue)
